...
1. **Сцена** — конкретный **линейный** человек в конкретный момент (менеджер, аккаунт, исполнитель задачи, оператор чата); боль про **деньги / план / крупный слив / войну отделов**, не про «аккуратность CRM».
2. **Платёжный тест** — ценность одной фразой без аналитического жаргона; фраза звучит как ставка («сорвём план», «сольём корпоратив», «бюджет в пустоту»), не как удобство.
3. **Зазор vs коробка** — не закрывается экраном / роботом / БП / фильтром / типовым отчётом / чатом / Excel.
4. **Тест ленивого менеджера** — неделю никто не заполняет ничего нового → польза из уже лежащих данных CRM/задач/чатов всё ещё есть. Иначе дисквал.
...
6. **Тест реализации:** за 30 секунд ясно *что кодируем* (placement + 2–4 REST + один экран/мастер). Если ответ «аналитическая модель для РОПа / симулятор / матрица / кабина» — **дисквал**.
7. **Не процесс РОПа:** идея не должна «выстраивать управление отделом». Покупатель и ежедневный пользователь — человек в рутине; РОП максимум вторичный бенефициар.

**Цель продукта:** снять **конкретную рутину** в момент работы (клик / мастер / блок в карточке), а не спроектировать новый управленческий контур.
...
В первую очередь: менеджер продаж, аккаунт, **внутренний** специалист той же компании (ТП, юрист, логист — сотрудник), участник чата сделки.  
**Не ICP:** внешний подрядчик / фрилансер «в задачах клиента» — редкий кейс, не строить продукт вокруг него.  
РОП/комдир — только если сам делает ту же рутину руками (не «смотрит дашборд»).  
Домены крутить **по кругу**: CRM ↔ Задачи ↔ Чаты (не залипать только в воронке сделок).  
...
2. **Роботизация (жёстко):** собирается ли job **одним роботом, цепочкой 2–3 роботов или коротким БП** (триггер оплаты → стадия → дело/уведомление/копия строк/проведение)? Да → дисквал. Формула «у робота нет этого поля, значит приложение» — **запрещена**, если админ всё равно собрал бы это автоматизацией воронки.
3. **Что остаётся продуктом:** новый killer‑экран / модель / объект работы, который **не сводится** к оркестрации стадий; либо вычисление/когорта/UI, которое роботы принципиально не выражают (не «уведомить», а изменить исход ритуала).
4. **Маркет:** 5–10 приложений; если ≥3 сильных уже закрывают тот же job — либо узкий клин «лучше/проще», либо дисквал.
5. **Ставка:** без приложения через 1–4 недели — план/касса/крупный клиент/война отделов? Нет → дисквал (гигиена).
6. **Ленивый менеджер:** данные уже в CRM/событиях? Да.
7. **Скоринг** A≥4, B≥3, C≥3.

В предложении в чат (G4) блок валидации всегда включает: **«Недоделка штатива: … / Доказательство: … / Почему не робот/БП (в т.ч. цепочка): … / Маркет: …»**.
...
1. **Заменитель роботов/БП** — job = «когда событие/стадия → проверить поле / уведомить / сменить стадию / создать дело / скопировать строки / провести документ». Тест админа: *нормальный внедряльщик закрыл бы это 1–3 роботами или коротким БП?* Да → **дисквал**, даже если «одного робота 1:1 нет» и нужна склейка 2 сущностей. Приложение ≠ упаковка автоматизации воронки.
1b. **Театр процесса для РОПа** — кабины, симуляторы «дойдём ли», матрицы handoff, наборы «дожим плана», рекомендатели наблюдателей «для отдела». Тест: *это приложение, которое линейный сотрудник жмёт в своей рутине, или консультант по внедрению продаж?* Второе → **дисквал**.
1c. **Непонятно что реализовать** — нет одного placement/экрана и короткого списка API. «Модель на 90 днях истории» как MVP без кнопки в рутине → **дисквал**.
1d. **Подрядчик в задачах клиента** — сценарий «внешний исполнитель живёт в задачах портала» → **дисквал** (редко). Задачи/чаты — про сотрудников одной компании.
1e. **Несколько ответственных на одном клиенте** — коллизии слотов/касаний/параллельных сделок «коллега тоже ведёт» → **дисквал**. В типовой компании **один ответственный на клиента**; такой театр редкий и раздражает.
2. Штатный дедуп.
...
6. Фейковый chat handoff / чтение текста сообщений / «добавить владельца в чат ОЛ».
...
Уроки: **114–118**; **269–273**; **285/286/288**; радары **274–313**; дисциплина **314–318**; авто‑гигиена **314–318b**; **анти‑робот‑пакет 314–318c**; **анти‑РОП‑театр 314–318d**; **анти‑подрядчик‑в‑задачах 314–318e**; **анти‑мульти‑owner 314–316f** (слот контакта занят коллегой, чужие касания, другие сделки компании) — обычно один ответственный на клиента; **318f** «позвать в наблюдатели+чат» — штатный B24.
...
- роботы / триггеры / БП (включая **цепочку** из нескольких);
- фильтры и штатные отчёты CRM;
- типовые экраны (дела, стадии, товары, чаты).

Если находится терпимый обход **или** очевидная настройка автоматизации продаж — **дисквал** (не предлагать).
...
2. Сцена / боль / цена — **линейная роль**, не РОП‑театр
3. Что делает (работа, не лог)
4. **Что кодируем:** placement + ключевые REST (обязательно)
5. Для кого (ежедневный пользователь)
6. Эффект
7. MVP (3–5 фич)
8. Чем НЕ штатив / робот / БП / отчёт
9. Как сейчас обходят
10. Валидация кратко: **недоделка штатива + доказательство** · почему не робот · A/B/C · 2–3 конкурента/дыры Маркета
11. Риски
12. Команды: `ок N` · `в беклог N` · `отклони N`
